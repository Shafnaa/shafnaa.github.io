...
class Generator {
  ...
  generate() {
    ...
    this.count = this.getRndInteger(1, 6);
    this.stems = this.getRndInteger(1, 10);
    this.edge = 0;
    ...
    for (let i = 0; i < this.count; i++) {
      ...
      let walker = new Walker({
        ...
        total: this.getRndInteger(500, 8000),
        x: centered ? 0 : Calc.rand(-1, 1),
        y: centered ? 0 : Calc.rand(-1, 1),
        dir: (i / this.count) * ((Math.PI * 2) / this.stems),
        speed: 0,
        delta: this.getRndInteger(1, 500) / 1000000,
        angleRange: this.getRndInteger(2, 18) / 1000,
        away: 0,
        depth: this.getRndInteger(0, 100) / 10,
        time: i * 1000,
      });
      ...
      let material = new MeshLineMaterial({
        ...
        color: new THREE.Color(
          `hsl(${
            360 +
            this.getRndInteger(0, 360) +
            Calc.map(
              i,
              0,
              this.count,
              -this.getRndInteger(0, 90),
              this.getRndInteger(0, 90)
            )
          }, 100%, ${this.getRndInteger(0, 100)}%)`
        ),
        depthTest: false,
        opacity: 1,
        transparent: true,
